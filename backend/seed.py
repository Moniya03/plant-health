...
from src.db import init_db, insert_reading, insert_analysis, update_plant_config, get_reading_count
...
async def main():
    ...
    for i in range(1440):
        ...
        # Light: sunrise 6am, peak noon, sunset 6pm, 0 at night
        if 6 <= hour <= 18:
            light_base = 5000 * math.sin(math.pi * (hour - 6) / 12)
        else:
            light_base = 0
        # Add occasional cloud (random dip)
        if random.random() < 0.05:
            light_base *= 0.4
        light = round(max(0, light_base + random.gauss(0, 50)), 0)
    ...
    await insert_analysis(
        ...
        recommendations=["Water the plant with 200ml", "Check drainage to prevent root rot"],
    ...
    await insert_analysis(
        ...
        status="healthy",
        issues=[],
        recommendations=["Plant is recovering well after watering. Maintain current care routine."],
        analysis_type="rule_based",
        model_used=None,
    )
    
    total = await get_reading_count()
    print(f"Inserted {readings_inserted} readings, 3 analyses, species: Monstera deliciosa")
